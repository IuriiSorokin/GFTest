...
void MaterialEffects::noiseBetheBloch(M7x7& noise, double mom, double betaSquare, double gamma, double gammaSquare) const
{
  ...
  if (kappa > 0.01) { // Vavilov-Gaussian regime
    ...
    assert(tailCutoff_ ==0 );// ensure GENFIT makes only one step;
    tailCutoff_ = 1;
    ...
    if (Nc > 50.) { // truncated Landau distribution
      ...
      static const double kSigmaAlpha[] = {2.83, 4.23, 10.16, 13.88,  15.76,  18.19,  22.33,  31.59,  54.6    };
      static const double kAlpha[]      = {0.90, 0.95,  0.99,  0.995,  0.996,  0.997,  0.998,  0.999,  0.9996 };
      ROOT::Math::Interpolator interpolator;
      interpolator.SetData(9, kSigmaAlpha, kAlpha );
      tailCutoff_ = tailCutoff_ > 0 ? 1 : interpolator.Eval( sigmaalpha ); // if GENFIT makes more than one step, then Gaussian regime is valid
      ...
      tailCutoff_ = tailCutoff_ > 0 ? 1 :  alpha; // if GENFIT makes more than one step, then Gaussian regime is valid
    }
  }
